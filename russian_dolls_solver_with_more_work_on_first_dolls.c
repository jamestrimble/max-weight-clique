...
void expand(struct Graph *g, struct VtxList *C, struct UnweightedVtxList *P,
        ...
        long *expand_call_count, bool prune_using_c)
{
    (*expand_call_count)++;
    ...
    for (int i=P->size-1; i>=0; i--) {
        int v = P->vv[i];
        if (prune_using_c && C->total_wt + c[v] <= incumbent->total_wt) break;
        ...
        expand(g, C, &new_P, c, incumbent, level+1, next_vtx_fun, colouring_type, expand_call_count, prune_using_c);
        vtxlist_pop_vtx(g, C);
        if (colouring_type==0) {
            bound -= g->weight[v];
        ...
        int colouring_type, int colouring_order, int vtx_ordering, struct VtxList *incumbent)
{
    ...
    for (int i=0; i<ordered_graph->n; i++) {
        bool i_is_small = i < ordered_graph->n * .5;
        ...
        for (int j=0; j<i; j++)
            if (ordered_graph->adjmat[i][j])
                P.vv[P.size++] = j;
        long cached_incumbent = incumbent->total_wt;
        if (i_is_small)
            incumbent->total_wt = 0;
        expand(ordered_graph, &C, &P, c, incumbent, 0, colouring_order ? first_set_bit : last_set_bit,
                colouring_type, expand_call_count, !i_is_small);
        printf("%d %ld\n", i, incumbent->total_wt);
        c[i] = incumbent->total_wt;
        incumbent->total_wt = (incumbent->total_wt > cached_incumbent) ? incumbent->total_wt : cached_incumbent;
//        if (!quiet)
